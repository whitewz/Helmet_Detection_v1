...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "安全帽佩戴实时检测系统"))
                ...
                for i, det in enumerate(pred):  # detections per image
                    if det is not None and len(det):
                        ...
                        for c in det[:, -1].unique():
                            ...
                            if "no" in self.names[int(c)]:  # 获得检测结果类名，判断==Not wearing properly
                                if self.tick == 0:
                                    ...
                                    image = ImageGrab.grab()
                                    now = get_now_time()
                                    image.save('D:/wgggg/images/'+str(now)+'.jpg','JPEG')
                                    self.tick = 70
                                else:
